...
abstract class Channel
{
    ...
    public function connect()
    {
        ...
        if (!$this->socket) {
            throw new NoConnectionException("Could not connect to Sonic [$errno]: $errmsg");
        }
        if ($this->receiveTimeout > 0) {
            stream_set_timeout($this->socket, $this->receiveTimeout);
        }
    ...
    protected function sendMessage(SonicMessage $message)
    {
        ...
        if (defined('__SONIC_CLIENT_DEBUG__') && __SONIC_CLIENT_DEBUG__) {
            echo('[' . $this->mode . ":SENT]: $messageStr");
        }
    }
